  ...
  async fetch(request, env, ctx) {
    ...
    if (blogMatch && !path.startsWith("/blog/category") && !path.startsWith("/blog/tag")) {
      ...
      try {
        ...
        if (exists) {
          dynamic = await renderBlogPost(slug, env);
        }
      } catch (e) {
        // D1 unavailable (e.g. daily row-read quota 7500). Fall through to static
    ...
    if (path === "/sitemap.xml") {
      // Try static sitemap-index.xml asset first (hyphen, generated by scripts/build_sitemaps.py)
      try {
      ...
      // Fallback: try sitemap_index.xml (underscore, Astro route)
      try {
        const r2 = await env.ASSETS.fetch(new URL("https://assets/sitemap_index.xml"));
        if (r2.ok) {
...
async function handleAdminMigrate(request, env) {
  ...
  const statements = [
    ...
    // D1 quota fix (2026-09-02): related-posts blog query burned 166M rows/day (32x quota 5M).
    // Composite index (service|city, iso_date DESC) makes ORDER BY use index — no full scan+sort.
    `CREATE INDEX IF NOT EXISTS idx_posts_meta_service_iso ON posts_meta(service, iso_date DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_posts_meta_city_iso ON posts_meta(city, iso_date DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_email_queue_status_sent ON email_queue(status, sent_at)`,
    // D1 quota round 2 (2026-09-02): COUNT generated_drafts WHERE status='draft' full-scanned
    // ~14k rows per run × 4.5/h = ~1.5M/day. Index makes it an index-only count of ~380 drafts.
...
// AI Model registry — diverifikasi 2026-08-30 via opencode.ai/docs/zen + live ai-test.
// REMOVED: x-preview-f-free, hy3-free (401 "Model not supported" — sudah dihapus Zen).
// ADDED: ling-3.0-flash-fin-free (free tier baru, terverifikasi docs Zen 30 Agu 2026).
// Nemotron = NVIDIA trial (fast/lightning, slow/ultra). 429 = rate-limit sementara, model tetap valid.
...
const ZEN_FREE_MODELS = [
  ...
  "ling-3.0-flash-fin-free",   // stealth, free (baru 2026-08-30)
...
async function handlePingSitemap(request, env) {
  ...
  if (env.GSC_SERVICE_ACCOUNT_JSON) {
    ...
    if (gscToken) {
      ...
      for (const sub of subSitemaps) {
        // Feedpath must be the FULL https URL of the sitemap (not prefixed with sc-domain:)
        const sitemapPath = `https://beriklan.co.id/${sub}`;
        try {
  ...
  if (gscToken) {
    for (const sub of subSitemaps) {
      const sitemapUrl = `https://beriklan.co.id/${sub}`;
      try {
  ...
  const sitemapIndex = `https://beriklan.co.id/sitemap-index.xml`;
...
async function handleNewsSitemap(request, env) {
  ...
  try {
    ...
    const articles = await env.DB.prepare(`
      SELECT slug, title, excerpt, iso_date, service, city
      FROM posts_meta
      WHERE iso_date >= datetime('now', '-36 hours')
      ORDER BY iso_date DESC
      LIMIT 10
    `).all();
...
${rows.map(a => `  <url>
    <loc>https://beriklan.co.id/blog/${escHtml(a.slug)}/</loc>
    <lastmod>${escHtml((a.iso_date || '').slice(0, 16))}Z</lastmod>
    <news:news>
      <news:publication>
        <news:name>Beriklan.co.id</news:name>
        <news:language>id</news:language>
      </news:publication>
      <news:publication_date>${escHtml((a.iso_date || '').slice(0, 16))}Z</news:publication_date>
      <news:title>${escHtml(a.title || a.slug)}</news:title>
      ${a.service ? `<news:keywords>${escHtml(a.service)}${a.city ? ',' + escHtml(a.city) : ''},digital marketing,${escHtml(a.title || '').slice(0, 80)}</news:keywords>` : ''}
    </news:news>
  </url>`).join('\n')}
</urlset>`;
...
async function _getBlogTpl(env) {
   ...
   try {
     // Pakai / (homepage) sebagai reference: punya Navbar, StickyCTA, Footer, FloatingWhatsApp,
     // CookieBanner semua lengkap. Sebelumnya coba /blog/ → tidak ada <footer> (blog index
     // cuma artikel list, footer tidak di-include). Split pakai </head> + <footer> (stabil).
     const refResp = await env.ASSETS.fetch(new Request('https://beriklan.co.id/'));
     ...
     const prefix = html.substring(0, headEnd + '</head>'.length) +
       '\n<body lang="id" class="scroll-smooth">\n';
     const suffix = html.substring(footerStart);
     const bodyOpenEnd = html.indexOf('<body') >= 0
     ...
     const bodyMid = bodyOpenEnd > 0 ? html.substring(bodyOpenEnd, footerStart) : '';
      ...
      const islands = allIslands.filter(s => /Navbar|StickyCTA|FloatingWhatsApp|CookieBanner/.test(s)).join('\n');
...
function _injectPageExtras(prefix, islands, bodyPre) {
  let p = prefix;
  if (!p.includes('pagead2.googlesyndication.com')) {
    p = p.replace('</head>', '<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-4438184351486735" crossorigin="anonymous"></script>\n</head>');
  }
  // Homepage's body scripts (Astro hydration: astro-island{display:contents} + Astro.only +
  // renderer controller) live BEFORE the first <astro-island>. Without them, the
  // injected islands stay as plain HTMLUnknownElement → no hydration → no Navbar.
  if ((bodyPre || islands) && !p.includes('astro:only')) {
    const tail = (bodyPre || '') + '\n' + (islands || '');
    p = p.replace(/<body[^>]*>/, (m) => m + '\n' + tail);
  }
  return p;
}
...
async function renderBlogPost(slug, env) {
  ...
  try {
    ...
    if (tpl) prefix = _injectPageExtras(prefix, tpl.islands, tpl.bodyPre);
